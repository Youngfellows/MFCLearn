...
CMFCCtrlDlg::CMFCCtrlDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFCCTRL_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CMFCCtrlDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER, &CMFCCtrlDlg::OnCustomdrawSlider)
END_MESSAGE_MAP()
...
void CMFCCtrlDlg::OnCustomdrawSlider(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	int nPos = m_slider.GetPos();
	CString str;
	str.Format("滑块位置:%d", nPos);
